...
function formatDmHeight(heightInDm: number): string {
  ...
  return heightInM >= 1
    ? `${heightInM.toFixed(1)} m`
    : `${(heightInM * 10).toFixed(1)} cm`;
}
function formatHgWeight(weightInHg: number): string {
  const weightInKg = weightInHg / 10;
  return weightInKg >= 1
    ? `${weightInKg.toFixed(1)} kg`
    : `${(weightInKg * 100).toFixed(1)} g`;
}

const SpeciesDetails = ({ details, speciesDetails }: SpeciesDetailsProps) => {
  const habitat = speciesDetails?.habitat?.name || 'Unknown';
  const genus =
    speciesDetails?.genera?.find((it: Genus) => it.language.name === 'en')
      ?.genus || 'Unknown';
  const generationStr = speciesDetails?.generation?.name;
  const generationNb = (generationStr.match(/generation-(.*)/) ||
    [])[1].toUpperCase();
  const description = speciesDetails?.flavor_text_entries?.find(
    (e: any) => e.language.name === 'en' && e.version.name === 'x'
  )?.flavor_text;
  return (
    <div className="species-details">
      ...
      {speciesDetails && (
        <>
          ...
          <p>Genus: {genus}</p>
          {description && (
            <p className="species-details__description">{description}</p>
          )}
        </>
      )}
    </div>
